...
export const makeMessage = (score: number): string => {
  let msg: string = "";
	if (score <= 4) {
			msg = mensajes.mensaje_entre_0_y_4
	}
	else if (score > 4 && score <= 5) {
			msg = mensajes.mensaje_entre_4_y_5
	}
	else if (score > 5 && score <= 7) {
			msg = mensajes.mensaje_entre_5_y_7
	}
	else if (score === 7.5) {
			msg = mensajes.mensaje_con_7_y_media
	}
	else {
			msg = mensajes.mensaje_con_8_o_mas
	}
 return msg;
}
